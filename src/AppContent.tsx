...
// Lazy load heavy analytics (recharts, jspdf, html2canvas)
...
export default function AppContent({ authState }: AppContentProps) {
  const { user, signOut } = authState;

  // Invisible router - URL always stays '/' but back/forward works
  const { currentView, trackerId, navigate } = useInvisibleRouter();
